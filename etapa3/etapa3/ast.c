...
void astCreateCode(ast* node){
    if(node){		
		switch(node->type){
            ...
            case AST_VEC:        { astPrintVec(node); break;}      
            case AST_FUNC_USE:        { astPrintFunUse(node); break;}   
...
void astPrintBinOp(ast* node, char* opSymbol){ 
     ...
     fprintf(outputFile,"%s = %s %s %s;\n",node->symbol->text,node->son[0]->symbol->text,opSymbol,node->son[1]->symbol->text);
...
void astPrintFunCall(ast* node){
    ...
    fprintf(outputFile,"%s = %s ( ",node->symbol->text,node->son[0]->symbol->text);
    
    for ( iterator = node->son[0]->son[0]; iterator!=0; iterator= iterator->son[1]){
    	if (iterator->son[0]) fprintf(outputFile,"%s", iterator->son[0]->symbol->text); 			
    	if (iterator->son[1] && iterator->son[1]->son[0]) fprintf(outputFile,", "); 
    }
    fprintf(outputFile," );\n"); 	
}
...
void astPrintReturn(ast* node){ 
    astCreateCode(node->son[0]);
    fprintf(outputFile,"return %s",node->son[0]->symbol->text);
}
...
void astPrintPrint(ast* node){ 
    if (node->symbol){
        fprintf(outputFile,"print %s",node->symbol->text);
    }
    else{
        if(node->son[0]->son[0]){
            astCreateCode(node->son[0]);
        }
        fprintf(outputFile,"print %s",node->son[0]->symbol->text);
    }
}
...
void astPrintVec(ast* node){
    astCreateCode(node->son[0]);
    fprintf(outputFile,"%s = %s [ %s ];\n",node->symbol->text,node->son[0]->symbol->text,node->son[0]->son[0]->symbol->text);
}
